...
export async function renderMap(root, data) {
  const source = JSON.parse(
    await readFile(join(root, 'data/exploration-map.json'), 'utf8'),
  );
  const geography = JSON.parse(
    await readFile(join(root, 'data/map-geography.json'), 'utf8'),
  );
  ...
  const metersPerLongitude =
    111320 * Math.cos((((bounds.north + bounds.south) / 2) * Math.PI) / 180);
  const metersWidth = (bounds.east - bounds.west) * metersPerLongitude;
  const height = Math.round(
    (width * (bounds.north - bounds.south) * 111320) / metersWidth,
  );
  const config = {
  ...
  for (const [index, mission] of missions.entries()) {
    if (
      ...
      !/^[a-z0-9-]+$/.test(mission.id) ||
      quizIds.has(mission.id)
    )
      throw new Error('Invalid or duplicate map quiz ID');
    quizIds.add(mission.id);
    const region = source.regions.find(
      (item) => item.id === mission.explorationRegionId,
    );
    const area =
      mission.explorationArea === undefined ? region : mission.explorationArea;
    const p =
      area &&
      projectPoint(area.latitude, area.longitude, bounds, width, height);
    if (
      !p ||
      p.x < 0 ||
      p.x > width ||
      ...
      throw new Error(`Invalid exploration area for ${mission.id}`);
    if (
      !/^assets\/(?:[a-z0-9-]+\/)*[a-z0-9-]+\.(svg|jpg|jpeg|png|webp)$/.test(
        mission.image,
      )
    )
      throw new Error('Invalid map clue image');
    ...
    const description =
      mission.explorationDescription ??
      region?.description ??
      '사진 속 특징이 있는 장소를 색칠된 범위 안에서 찾아보세요.';
    if (typeof description !== 'string' || !description.trim())
      throw new Error(`Invalid exploration description for ${mission.id}`);
    config.quizzes.push({
      id: mission.id,
      number: index + 1,
      regionId: mission.explorationRegionId ?? null,
      description,
      clueImage: `data:${mime};base64,${bytes.toString('base64')}`,
      clueAlt: mission.imageAlt,
      x: fixed(p.x),
      y: fixed(p.y),
      radius: fixed(area.radiusMeters * pixelsPerMeter),
    });
  }
  const point = ([longitude, latitude]) =>
    projectPoint(latitude, longitude, bounds, width, height);
  const path = (feature, close = false) =>
    feature.geometry
  ...
  const allowed = (feature) =>
    feature.tags?.foot !== 'no' &&
    (feature.tags?.foot === 'yes' ||
      !['private', 'no'].includes(feature.tags?.access));
  const paths = [];
  for (const park of geography.parks)
    paths.push(`<path d="${path(park, true)}" class="park"/>`);
  ...
  for (const road of geography.roads.filter(allowed)) {
    const category = /^(primary|secondary|tertiary)/.test(road.tags.highway)
      ? 'main-road'
      : ['footway', 'steps', 'path'].includes(road.tags.highway)
        ? 'walking-path'
        : 'alley';
    paths.push(
      `<path d="${path(road)}" class="road-edge ${category}"/><path d="${path(road)}" class="road ${category}"/>`,
    );
  }
  const landmark = (id, label, kind) => {
    ...
    const decoration =
      kind === 'station'
        ? '<rect x="-22" y="-26" width="44" height="46" rx="8"/><path d="M-12-15H12V0H-12ZM-13 29l6-9m20 9-6-9"/>'
        : '';
  ...
  const zones = config.quizzes.map(
    (quiz) =>
      `<g id="zone-${xml(quiz.id)}" class="zone" data-zone="${xml(quiz.id)}" hidden><circle cx="${quiz.x}" cy="${quiz.y}" r="${quiz.radius}"/><g transform="translate(${quiz.x} ${fixed(quiz.y - quiz.radius + 27)})"><rect x="-60" y="-24" width="120" height="46" rx="23"/><text y="8">문제 ${quiz.number}</text></g></g>`,
  );
  const cards =
    config.quizzes
      .map((quiz) => {
      ...
      .join('') ||
    '<p id="quiz-empty" class="empty-card">아직 등록된 현장 문제가 없어요. 문제가 준비되면 이곳에서 탐험을 시작할 수 있어요.</p>';
  const park = geography.parks.find((item) => item.tags?.name === '자유공원');
  let parkLabel = '';
  if (park) {
    const inside = park.geometry
      .map(point)
  ...
  let output = await readPageSource(root, 'map');
  const locationCore = (
    await readFile(join(root, 'location.mjs'), 'utf8')
  ).replace(/^export /gm, '');
  for (const [slot, value] of [
    ['<!--__MAP_SVG__-->', svg],
    ['<!--__QUIZ_CARDS__-->', cards],
    ['/*__MAPDATA__*/ null', serializeJson(config)],
    ['/*__LOCATION__*/', locationCore],
  ])
...
export async function buildMap(root, data) {
  ...
  console.log(
    `Built exploration map: ${data.missions.filter((mission) => mission.sceneKind === 'field').length} field quizzes.`,
  );
}

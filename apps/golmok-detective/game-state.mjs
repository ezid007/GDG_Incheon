...
function hasExpectedSolvedIds(state, missions, count) {
  return state.solvedIds.length === count
...
export function restoreState(raw, missions, version) {
  ...
  if (!raw || typeof raw !== 'object' || Array.isArray(raw) || raw.version !== version) return fallback();
  const state = raw.state;
  if (!state || typeof state !== 'object' || Array.isArray(state)) return fallback();
  if (!['start', 'mission', 'complete'].includes(state.screen)
    ...
    || typeof state.hintVisible !== 'boolean') return fallback();

  if (state.screen === 'start') return fallback();

  if (!Array.isArray(missions) || missions.length === 0
    || state.missionIndex < 0 || state.missionIndex >= missions.length) return fallback();
  ...
  if (state.screen === 'complete') {
    return state.missionIndex === missions.length - 1
      ...
      && state.feedback === 'idle'
      && state.hintVisible === false ? copyState(state) : fallback();
  }

  const mission = missions[state.missionIndex];
  const selectionIsValid = state.selectedOptionId === null
    || mission.options.some(option => option.id === state.selectedOptionId);
  if (!selectionIsValid) return fallback();

  if (state.feedback === 'correct') {
    if (state.selectedOptionId !== mission.answerId
      || !hasExpectedSolvedIds(state, missions, state.missionIndex + 1)) return fallback();
  } else {
    if (!hasExpectedSolvedIds(state, missions, state.missionIndex)) return fallback();
    if (state.feedback === 'wrong'
      && (state.selectedOptionId === null || state.selectedOptionId === mission.answerId)) return fallback();
  }
...
export function transition(state, action, missions) {
  ...
  if (action.type === 'START') {
    return state.screen === 'start' && missions.length > 0
      ? { ...initialState(missions), screen: 'mission' } : next;
  }
  ...
  switch (action.type) {
    case 'SELECT':
      ...
      return next;
    case 'CHECK':
      if (state.selectedOptionId === null || state.feedback === 'correct') return next;
      next.feedback = state.selectedOptionId === mission.answerId ? 'correct' : 'wrong';
      if (next.feedback === 'correct' && !next.solvedIds.includes(mission.id)) next.solvedIds.push(mission.id);
      return next;
    case 'TOGGLE_HINT':

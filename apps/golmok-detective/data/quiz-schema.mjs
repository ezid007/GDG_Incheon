import { isPublicImage } from '../mission-validation.mjs';
...
class FieldQuiz {
  ...
  static validate(data) {
    ...
    if (data.quizImage !== undefined && !isPublicImage(data.quizImage)) {
      throw new Error(`Invalid quizImage path: ${data.quizImage}`);
    }
    if (data.answerImage !== undefined && !isPublicImage(data.answerImage)) {
...
class ExampleQuiz {
  ...
  static validate(data) {
    ...
    if (!Array.isArray(data.options) || data.options.length !== 3) {
      throw new Error('options must be array of 3 items');
    }
...
class QuizSupportData {
  ...
  static validate(data) {
    ...
    for (const review of reviews) {
      if (quizKeys.has(review.quizKey)) throw new Error(`Duplicate quiz key: ${review.quizKey}`);
      quizKeys.add(review.quizKey);
      if (review.options.filter(option => option.label === review.expectedAnswer).length !== 1) {
        throw new Error(`Quiz ${review.quizKey} must have exactly one reviewed answer.`);
      }
    }

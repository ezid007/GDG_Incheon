...
export const isPublicImage = value => typeof value === 'string'
...
export function validateMissionData(data) {
  ...
  for (const mission of data.missions) {
    if (!mission || !hasText(mission.id) || ids.has(mission.id)
      || !Object.hasOwn(optionCounts, mission.sceneKind)) {
      throw new Error('Invalid mission identity or source.');
    }
    ids.add(mission.id);
    if (mission.sceneKind === 'field' && !['songwol', 'chinatown'].includes(mission.explorationRegionId)) {
      throw new Error('Field missions require a supported exploration region: songwol or chinatown.');
    }
    const expectedCount = optionCounts[mission.sceneKind];
    if (!Array.isArray(mission.options) || mission.options.length !== expectedCount) {
      throw new Error(`${mission.sceneKind} missions require exactly ${expectedCount} choices.`);
    }
    if (!mission.options.every(option => option && hasText(option.id) && hasText(option.label))) {
      throw new Error('Option IDs and labels must be non-empty strings.');
    }
    if (new Set(mission.options.map(option => option.id.trim())).size !== expectedCount
      || new Set(mission.options.map(option => option.label.trim())).size !== expectedCount) {
      throw new Error('Option IDs and labels must be unique.');
    }
    if (mission.options.filter(option => option.id === mission.answerId).length !== 1) {
      throw new Error('The answer ID must match exactly one choice.');
    }
    for (const key of requiredText) {
      if (!hasText(mission[key])) throw new Error('Missing mission text: ' + key);
    }
    if (!mission.evidence || !['x', 'y'].every(key => Number.isFinite(mission.evidence[key])
      && mission.evidence[key] >= 0 && mission.evidence[key] <= 100)) {

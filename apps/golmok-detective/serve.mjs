...
const ROUTES = new Map([
  ...
  [
    '/assets/share-qr-20260913.jpg',
    ['assets/share-qr-20260913.jpg', 'image/jpeg'],
  ],
  [
    '/assets/example-window.svg',
    ['assets/example-window.svg', 'image/svg+xml; charset=utf-8'],
  ],
  [
    '/assets/example-sign.svg',
    ['assets/example-sign.svg', 'image/svg+xml; charset=utf-8'],
  ],
  [
    '/assets/example-direction.svg',
    ['assets/example-direction.svg', 'image/svg+xml; charset=utf-8'],
  ],
]);
...
const HEADERS = {
  ...
  'Content-Security-Policy':
    "default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'none'; object-src 'none'; base-uri 'none'; form-action 'none'; frame-ancestors 'none'",
};
...
if (isDirectRun) {
  try {
    ...
    server.on('error', () => {
      console.error(
        'Unable to start the game server. Check whether the port is available.',
      );
      process.exitCode = 1;
    });

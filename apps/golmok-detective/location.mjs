...
export function assessLocation(
  ...
  if (accuracy > LOCATION_MAX_ACCURACY_METERS)
    return { status: 'inaccurate', ...measurement };
  if (
  ...
  const metersPerLongitudeDegree =
    METERS_PER_LATITUDE_DEGREE * Math.cos((midLatitude * Math.PI) / 180);
  ...
  if (
    !point ||
    !Number.isFinite(mapWidthMeters) ||
    mapWidthMeters <= 0 ||
    !Number.isFinite(mapHeightMeters) ||
    mapHeightMeters <= 0 ||

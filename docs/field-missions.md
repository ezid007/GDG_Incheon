...
현재 실제 문제는 3개입니다. 원본은 `apps/golmok-detective/data/quiz-support.json`, 사진은 `public/assets/quiz/`에서 관리합니다. 정답과 사진 역할은 사용자·출제자가 확정한 내용을 유지합니다.
...
사용하지 않는 인화문 힌트 사진과 동화마을 중복 사진은 앱의 `reference/unused-photos/`로 보존했습니다. 스마트폰 첨부 원본은 `.codex-remote-attachments/`에 있으며 삭제하거나 그대로 공개 폴더 전체에 복사하지 않습니다.
